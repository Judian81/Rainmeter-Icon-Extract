...
for /f "tokens=* delims=" %%f in ('dir /b/a-d /b %videofile2%') do (
  if not exist "%cachedir1%%%f.png.txt" (
    call "%skinaddon2%\IconExtract.exe" "-safe-ffmpeg-wrapper-video" "%browse2%%%f"
  )
)

rem call "%skinaddon2%\IconExtract.exe" "-safe-ffmpeg-wrapper-video" "c:\programdata\2cool\picture.png"
for /f "tokens=* delims=" %%f in ('dir /b/a-d /b %imagefile2%') do (
  if not exist "%cachedir1%%%f.png.txt" (
...
for /f "tokens=* delims=" %%f in ('dir /b/a-d /b %musicfile2%') do (
  if not exist "%cachedir1%%%f.png.txt" (
